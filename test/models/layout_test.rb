...
class LayoutTest < ActiveSupport::TestCase
  ...
  test "add field one" do
    ...
    expected = [{"http://example.com/0"=>"zero"}, {"http://example.com/1"=>"one"}]
  ...
  test "make turtle" do
    expected = "<http://example.com/0>  <http://culture-in-time.org/ontology/order> 0 ; <http://culture-in-time.org/ontology/name> \"zero\" .  "  
  ...
  test "make turtle with direction" do
    @layout.delete_field("http://example.com/0")
    @layout.add_field("http://example.com/1","one","Reverse")
    expected = "<http://example.com/1>  <http://culture-in-time.org/ontology/order> 0 ; <http://culture-in-time.org/ontology/name> \"one\" ; <http://culture-in-time.org/ontology/direction> <http://culture-in-time.org/ontology/Reverse> .  "  
    assert_equal  expected, @layout.turtle
  end
end